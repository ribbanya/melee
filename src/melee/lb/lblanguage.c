...
{
    return gmMainLib_GetSaveData()->saved_language;
}

void lbLang_SetSavedLanguage(enum_t language)
{
    if (language >= 0 && language < LANG_COUNT) {
        gmMainLib_GetSaveData()->saved_language = language;
    }
}

bool lbLang_IsSavedLanguageJP(void)
{
    return gmMainLib_GetSaveData()->saved_language == LANG_JP ? true : false;
}

bool lbLang_IsSavedLanguageUS(void)
{
    return gmMainLib_GetSaveData()->saved_language == LANG_US ? true : false;
}
