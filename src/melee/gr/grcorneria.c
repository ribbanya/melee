...
void grCorneria_801DCCFC(void)
{
    ...
    imin = yakumono_param->x3C;
    imax = yakumono_param->x40;
    if (imax > imin) {
        s32 range = imax - imin;
    ...
    if (Stage_80225194() != 0x46) {
        if (!grCn_804D69B0) {
            if (arwing_gobjs[0] == NULL) {
                ...
                if (grCn_804D69A8 <= 0) {
...

                    {
                        ...
                        if (*(groups = &arwing_groups[0]) == 4) {
                            ...
                            group = HSD_Randf() > yakumono_param->x4C ? 1 : 4;
                        }
                ...
                grCn_804D69A8 = grCn_RandRange((s32) yakumono_param->x48,
                                               (s32) yakumono_param->x44);
            }
        } else {
            {
...
void grCorneria_801DDAC4(Ground_GObj* gobj)
{
    ...
    arwing_gobjs[gp->u.arwing.xC8 = grCn_804D69A4] = gobj;
    {
        s32 idx = arwing_groups[gp->u.arwing.xC8];
        ...
        if (arwing != NULL) {
            GET_GROUND(arwing)->x10_flags.b2 = 0;
            {
                ...
                if (agp != NULL) {
                    agp->u.arwing.xC8 = gp->u.arwing.xC8;
                }
            }
        }
    }
    scale = Ground_801C0498();
    HSD_JObjSetScaleX(jobj, scale * yakumono_param->x70);
    HSD_JObjSetScaleY(jobj, scale);
    HSD_JObjSetScaleZ(jobj, scale);
    gp->u.arwing.xD4 = 1;
    gp->u.arwing.xD8 = 0.0f;
}
...
void grCorneria_801DDDA8(HSD_GObj* gobj, Vec3* vec)
{
    ...
    if (gobj != NULL) {
        gp = gobj->user_data;
        gp2 = Ground_GetMapGObj(3)->user_data;
...
void grCorneria_801DDE88(HSD_GObj* gobj)
{
    ...
    if (gobj != NULL) {
        ...
        idx = arwing_types[gp2->u.arwing.xC8];
...
void grCorneria_801DE024(Ground_GObj* gobj)
{
    ...
    HSD_JObj* jobj = GET_JOBJ(gobj);
    Vec3 translate;
    HSD_GObj* ship_gobj = Ground_GetMapGObj(3);
    ...
    {
        if (arwing_gobjs[gp->u.arwing.xC8] == gobj) {
            if (gp->u.arwing.xD4 == 1) {
                gp->u.arwing.xD4 = 0;
                {
...
void grCorneria_Arwing_801DE4C0(Ground_GObj* gobj)
{
    ...
    group = arwing_groups[gp->u.arwing.xC8];
...
void grCorneria_801DE8E4(Ground_GObj* gobj)
{
    ...
    case 9: {
        HSD_GObj* arwing = grCorneria_801DD534(
            grCn_803E217C[arwing_groups[gp->u.corneria2.xC8]]);
    ...
    case 13: {
        Ground_AttachStarFoxArwingModel(gobj, gp,
                                        arwing_gobjs[gp->u.corneria2.xC8], 4);
        break;
    }
    default:
    ...
    HSD_JObjSetScaleX(jobj, scale * yakumono_param->x70);
    HSD_JObjSetScaleY(jobj, scale * yakumono_param->x70);
    HSD_JObjSetScaleZ(jobj, scale * yakumono_param->x70);
    Ground_ResetStarFoxArwingState(gp);
    gp->u.corneria2.xF8 = (s32) yakumono_param->x68;
    gp->u.corneria2.xFC = 0;
    gp->u.corneria2.x100 = HSD_Randi(2);
}
...
static inline int get_grp_off(Ground_GObj* gobj)
{
    s32 grp_off = -1;
    {
        ...
        int g = groups[GET_GROUND(gobj)->u.corneria2.xC8];
        switch (g) {
        case 0:
            break;
        case 1:
...
void grCorneria_801DED50(Ground_GObj* gobj)
{
    ...
    if (arwing_gobjs[gp->u.corneria2.xC8] != NULL) {
        if (arwing_groups[gp->u.corneria2.xC8] == 4) {
        ...
        case 9: {
            HSD_JObjSetRotationY(jobj, -1.5707964f);
            switch (gp->u.corneria2.xF4) {
            case 0:
                if (gp->u.corneria2.xF8 <= 0) {
                    s32 new_state = HSD_Randi(5) + 1;
                    int group_offset = -1;
                    gp->u.corneria2.xF4 = new_state;
                    {
                        int group =
                            arwing_groups[GET_GROUND(gobj)->u.corneria2.xC8];
                        switch (group) {
                        case 0:
                            break;
                        case 1:
                    ...
                    {
                        int anim_arg =
                            grCn_803E214C[gp->u.corneria2.xF4][group_offset];
                        grAnime_801C8098(
                            gobj,
                            grCn_803E21B0[arwing_groups[gp->u.corneria2.xC8]],
            ...
            {
                if ((arwing = (HSD_JObj*) arwing_gobjs[gp->u.corneria2.xC8])) {
                    Vec3 arwing_pos;
                    arwing_gp = get_arwing_ground((Ground_GObj*) arwing);
                    ship_gp = GET_GROUND(Ground_GetMapGObj(3));
                    lb_8000B1CC(Ground_801C3FA4((Ground_GObj*) arwing, 4),
                                NULL, &arwing_pos);
                    {
                        int* types = arwing_types;
                        int atype = types[arwing_gp->u.corneria2.xC8];
            ...
            lb_8000B1CC(
                Ground_801C3FA4(
                    gobj, grCn_803E21B0[arwing_groups[gp->u.corneria2.xC8]]),
                NULL, &pos);
            {
                f32 rot_z;
                f32 angle;
                {
                    HSD_GObj* item = gp->u.corneria2.xDC;
                    if (item != NULL) {
        ...
        case 13: {
            ...
            {
                if ((far_arwing = arwing_gobjs[gp->u.corneria2.xC8])) {
                    Vec3 arwing_pos;
                    ...
                    lb_8000B1CC(Ground_801C3FA4(far_arwing, 4), NULL,
                                &arwing_pos);
                    {
                        s32 atype = arwing_types[arwing_gp->u.corneria2.xC8];
                        pos.x = ship_gp->u.corneria.offset_x +
                                (-arwing_pos.z + lbl_803E2068[atype].x);
                        pos.y = arwing_pos.y + lbl_803E2068[atype].y;
                        pos.z = arwing_pos.x + lbl_803E2068[atype].z;
                ...
                gp->u.corneria2.xFC != 0)
            {
                ...
                lbAudioAx_800237A8(0x55734, 0x7F, 0x40);
                {
                    if (get_grp_off(gobj) == 1) {
                        ...
                        if (gp->u.corneria2.x100 != 0) {
                            it_802E7654(gobj, Ground_801C3FA4(gobj, 5), &pos,
                                        1, 3, yakumono_param->x70);
                        } else {
        ...
        gp->u.corneria2.xF0 += 1;
        return;
    }
    {
        HSD_GObj* item;
        item = gp->u.corneria2.xE0;
        (void) item;
        if (item != NULL) {
            Ground_801C4A08(item);
        }
        item = gp->u.corneria2.xE4;
        if (item != NULL) {
            Ground_801C4A08(item);
        }
        item = gp->u.corneria2.xE8;
        if (item != NULL) {
            Ground_801C4A08(item);
        }
        item = gp->u.corneria2.xEC;
        if (item != NULL) {
            Ground_801C4A08(item);
        }
        Ground_801C4A08(gobj);
...
void grCorneria_801DFC98(Ground_GObj* gobj)
{
    ...
    {
        HSD_JObjAddTranslationX(jobj, yakumono_param->x88);
                 ...
                 f32 damage)
{
    ...
    if (gr->u.corneria.x114 >= yakumono_param->x38) {
...
void grCorneria_801E1348(Ground_GObj* gobj)
{
    ...
    case 0:
        if (gp->u.corneria.x110 == 0) {
            if (HSD_Randf() <= yakumono_param->x24) {
                gp->u.corneria.x119 = 0;
                gp->u.corneria.x118 = 1;
                gp->u.corneria.x10C = yakumono_param->x28;
                Ground_801C53EC(0x55739);
            } else {
                s32 imax;
                s32 imin = yakumono_param->x30;
                imax = yakumono_param->x34;
                if (imax > imin) {
                    s32 range = imax - imin;
                    imax = imin + (range != 0 ? HSD_Randi(range) : 0);
                } else if (imax < imin) {
                ...
                gp->u.corneria.x10C = yakumono_param->x2C;
    ...
    case 1:
        if (gp->u.corneria.x10C == 0) {
            ...
            gp->u.corneria.x10C = yakumono_param->x20;
    ...
    case 3:
        ...
        {
            s32 rand_range = yakumono_param->x18;
            s32 range = yakumono_param->x18;
            gp->u.corneria.x110 =
                (s32) (grCn_GetX1C() +
...
void grCorneria_801E1970(Ground_GObj* gobj)
{
    ...
    if (gp->u.corneria.x104 <= 0) {
        f32 randf = HSD_Randf();
        f32 a = yakumono_param->x4 - yakumono_param->x0;
        gp->u.corneria.x104 = a * randf + yakumono_param->x0;
        randf = HSD_Randf();
        gp->u.corneria.xF4 = yakumono_param->x8 * (2.0f * (randf - 0.5f));
        randf = HSD_Randf();
        gp->u.corneria.xF8 = yakumono_param->x8 * (2.0f * (randf - 0.5f));
    }
    ...
    gp->u.corneria.offset_y.val += gp->u.corneria.xF0;
    {
        ...
        f32 limit = yakumono_param->x10;
        if (offset > limit) {
            gp->u.corneria.xF4 = -yakumono_param->x8;
    ...
    }
    {
        ...
        f32 limit = yakumono_param->x14;
        if (offset > limit) {
            gp->u.corneria.xF8 = -yakumono_param->x8;
        } else if (offset < -limit) {
            gp->u.corneria.xF8 = yakumono_param->x8;
        }
    }
...
{
    if (stage_info.grkind == Gr_Kind_Corneria ||
        stage_info.grkind == Gr_Kind_Venom)
    {
        return true;
    }
    return false;
...
{
    if (stage_info.grkind == Gr_Kind_Corneria && line_id != -1) {
