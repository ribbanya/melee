...
bool ftCo_800A2718(mp_UnkStruct0* arg0)
{
    ...
    }
    {
             ...
             cur = cur->next)
        {
            ...
            if (island == mpIsland_8005AB54(cur_ip->x378_itemColl.floor.index))
            {
                return true;
            }
        }
    ...
    if (result != 0) {
        {
            ...
            if (floor_x <
                ...
                floor_y >
                    Stage_GetBlastZoneTopOffset() - temp_data->half_height)
            {
                oob = 1;
            } else {
                oob = 0;
...
static inline s32 ftCo_800A4038_inline1(float x, float y, Vec3* arg_floor_pos,
...
bool ftCo_800A6FC4(Fighter* fp, Vec3* arg1, Vec3* arg2)
{
         ...
         island = island->next)
    {
        ...
        mx = 0.5f * (a.x + b.x);
        my = 0.5f * (a.y + b.y);
        result = mpCheckFloor(mx, 10.0 + my, mx, my - 10.0, 0.0f, &floor_pos,
        ...
        line = 0;
        px = a.x + 5.0;
        blocked = mpCheckFloor(px, a.y + 10.0, px, a.y - 10.0, 0.0f,
                               &floor_pos, &line_id, &flags, &floor_normal, -1,
                               -1, -1, NULL, (Fighter_GObj*) line);
        if (blocked != 0) {
            result = line_id;
        ...
        line = 0;
        px = b.x - 5.0;
        blocked = mpCheckFloor(px, b.y + 10.0, px, b.y - 10.0, 0.0f,
                               &floor_pos, &line_id, &flags, &floor_normal, -1,
                               -1, -1, NULL, (Fighter_GObj*) line);
        if (blocked != 0) {
            result = line_id;
        ...
        item->ground_or_air != GA_Air)
    {
        ...
        if (!ftCo_800A2718_dontinline(island)) {
            ...
            line_id = -1;
            {
                ...
                f32 top = 1.0f + iy;
                f32 ix2 = ix;
                found = mpCheckFloor(ix2, top, ix, bottom, 0.0f, &floor_pos,
...
static inline s32 ftCo_800A8940_inline0(s32 result_in, s32 blocked_in,
                                        const int* line_id_ptr)
{
    int line_id;
    s32 blocked = blocked_in;
    s32 result = result_in;
    if (result != 0) {
        line_id = *line_id_ptr;
...
void ftCo_800A8940(Fighter* fp)
{
    ...
    result = ftCo_800A8940_inline0(result, blocked, &line_id);
    if (result != 0) {
...
void ftCo_800A9CB4(Fighter* fp)
{
    ...
    if (terminal_time <= 0.0f) {
        ...
        y_pos = fp->cur_pos.y +
                (fp->pos_delta.y * x_time - 0.5 * (*grav_p * sqrt_x_time));
    } else {
    ...
    if (x_time < 0.0) {
        ...
        if (y_pos < data->x54.y) {
            ...
            ftCo_800B4778(fp, CpuCmd_LstickXTowardDestinationClamped,
                          stick_clamp, 0x7F);
        } else {
            ftCo_800B46B8(fp, CpuCmd_SetLstickX, 0);
        }
    ...
    if (found != 0) {
        ...
        if (floor_x < fp->cpu.half_width + Stage_GetBlastZoneLeftOffset() ||
            floor_x >
                Stage_GetBlastZoneRightOffset() - temp_data->half_width ||
            floor_y <
                temp_data->half_height + Stage_GetBlastZoneBottomOffset() ||
            floor_y > Stage_GetBlastZoneTopOffset() - temp_data->half_height)
        {
            oob = 1;
        } else {
            oob = 0;
